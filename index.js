...
break
           ...
           case 'hobby':
				...
				case 'gay':
					if (!isGroup) return reply(mess.only.group)
						membr = []
						const gay11 = groupMembers
						const gay21 = groupMembers
						const gay11 = gay11[Math.floor(Math.random() * gay11.length)]
						const gay21 = gay21[Math.floor(Math.random() * gay21.length)]
						...
						membr.push(gay11.jid)
						membr.push(gay21.jid)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS GrabiieBot 👑* 」\n\`\`\``
					no = 0
